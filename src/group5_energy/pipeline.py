...
def run_pipeline() -> PipelineResult:
    ...
    half_features = prepare_half_hourly_frame(
    ...
    half_model, half_metrics, half_valid_predictions = fit_and_evaluate(
        half_features,
    ...
    daily_model, daily_metrics, daily_valid_predictions = fit_and_evaluate(
        daily_features,
        frequency="daily",
...
def load_hourly_weather() -> pd.DataFrame:
    ...
    return df.drop_duplicates("WeatherHour").sort_values("WeatherHour")
...
def load_temperatures() -> pd.DataFrame:
    ...
    return df.drop_duplicates("DateTime").sort_values("DateTime")
...
def load_daily_weather() -> pd.DataFrame:
    ...
    return df[keep_cols].drop_duplicates("Date").sort_values("Date")
...
def fit_and_evaluate(
    ...
    return tree_model, metrics, valid_predictions
...
def write_core_outputs(
    ...
    write_prediction_table(
        half_csv,
        half_predictions,
        PROCESSED_CSV_DIR / "group_5_half_hourly_predict.csv",
        PROCESSED_PARQUET_DIR / "group_5_half_hourly_predict.parquet",
    )
    write_prediction_table(
        daily_csv,
        daily_predictions,
        PROCESSED_CSV_DIR / "group_5_daily_predict.csv",
        PROCESSED_PARQUET_DIR / "group_5_daily_predict.parquet",
    )
...
def write_report(half_features: pd.DataFrame, daily_features: pd.DataFrame, metrics: pd.DataFrame) -> None:
    ...
    best_half = best_model(metrics, "half_hourly")
    best_daily = best_model(metrics, "daily")
