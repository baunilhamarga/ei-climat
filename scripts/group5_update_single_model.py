...
def update_frequency(
    *,
    ...
    selected_model = selected_model_after_metric_update(frequency, metric_rows)
...
def update_all_model_predictions(*, model_name: str, forecast: pd.DataFrame, time_col: str, path: Path) -> pd.DataFrame:
    ...
    out["_model_order"] = out["model"].map({model: idx for idx, model in enumerate(TARGETED_TRAINABLE_MODELS)}).fillna(99)
    out = out.sort_values(["_model_order", "model", "Acorn", time_col]).drop(columns="_model_order")
    return out.reset_index(drop=True)


def selected_model_after_metric_update(frequency: str, new_rows: pd.DataFrame) -> str:
    ...
    return selected_trainable_model_from_metrics(metrics, frequency)


def selected_trainable_model_from_metrics(metrics: pd.DataFrame, frequency: str) -> str:
    subset = metrics[
        (metrics["frequency"] == frequency)
        & (metrics["acorn"] == "ALL")
        & (metrics["model"].isin(TARGETED_TRAINABLE_MODELS))
    ].dropna(subset=["rmse"])
    if subset.empty:
...
def sync_selected_model_artifact(frequency: str, model_name: str) -> None:
    source = model_storage_path(frequency, model_name, selected=False)
    destination = model_storage_path(frequency, model_name, selected=True)
    ...
    summary.update(
        {
            ...
            "active_trainable_models": list(TARGETED_TRAINABLE_MODELS),
            "short_term_model_path": str(model_storage_path("half_hourly", half_selected, selected=True)),
            "medium_term_model_path": str(model_storage_path("daily", daily_selected, selected=True)),
            "short_term_model_paths": model_paths("half_hourly"),
            "medium_term_model_paths": model_paths("daily"),
...
def model_paths(frequency: str) -> dict[str, str]:
    return {name: str(model_storage_path(frequency, name, selected=False)) for name in TARGETED_TRAINABLE_MODELS}
...
def order_validation_columns(df: pd.DataFrame) -> pd.DataFrame:
    preferred = ["frequency", "timestamp", "Acorn", "actual", *MODEL_ORDER]
